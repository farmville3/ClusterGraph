...
function nodeInfo(params) {
  ...
  _.forEach(nodes_array, function(i){
    if (i.id === params.nodes[0]){
      node_label = i.label.replace("\n","")
    }
  })
  ...
  // out += "\n<br><button onclick='removeSelectedNode("+params.nodes[0]+")'>Delete node</button>" 
  return out;
}
...
function edgeInfo(params) {
  // console.log("<button onclick='removeSelectedEdge("+params.edges[0]+")'>Delete edge</button>")
...
function draw() {
  ...
  network.on("selectNode", function (params) {
    ...
    // network.stabilization.options.stabilization = false;
    console.log(network.physics);
  ...
  network.on("deselectNode", function (params) {
    $('#selection').html("");
    // document.getElementById("menu").style.resize = "none";
    // document.getElementById("menu").style.width = "300px";
    document.getElementById("menu").style.height = "";
  });
  // network.on("selectEdge", function (params) {
  //   // console.log('selectEdge Event');
  //   $('#edge').html(edgeInfo(params));
  // });
  ...
  network.on("startStabilizing", function() {
    Pace.start()
    console.log("statStabilizing start pace")
  });
  network.once("stabilizationIterationsDone", function() {
    Pace.stop()
    console.log("stop pace stabilizationIterationsDone")
  });
  // network.on("beforeDrawing",function(){
  //   Pace.start
  //   console.log("started pace")
  // });
  ...
  //   Pace.start
  //   console.log("started pace initredraw")
  // })
}
...
$(slider_node).rangeslider({
  ...
  onSlideEnd: function(position, value) {
    ...
    if (value < currentslider_node) {
      showEdges(nodes_range[value])
      showNodes(nodes_range[value])
    } else if (value > currentslider_node) {
      hideEdges(nodes_range[value])
      hideNodes(nodes_range[value])
    }
...
$(slider_edge).rangeslider({
  ...
  onSlideEnd: function(position, value) {
    ...
    // network.stabilize();
    Pace.start()
    console.log("start pace onSlideEnd")
    // need a split distribution of 10 cutoffs
    ...
    Pace.stop()
    console.log("stop pace onSlideEnd")
  }
});
